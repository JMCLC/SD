		...
		try (Scanner scanner = new Scanner(System.in)) {
			...
			do {
				...
				do {
					...
					play_res = null;
					try {
						play_res =  stub.play(PlayRequest.newBuilder().setRow(row).setColumn(column).setPlayer(player).build()).getResult();
						if (play_res == PlayResult.RESTART) {
							System.out.println("How boring, restarting game");	
							player = 0;
    					} else if (play_res != PlayResult.SUCCESS) {
